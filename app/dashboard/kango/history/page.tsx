// app/dashboard/kango/history/page.tsx
...
import {
  deleteKangoHistory,
  deleteAllKangoHistory,
  getKangoHistory,
  type KangoHistoryItem,
} from "@/lib/kangoHistory";

type YearFilter = "all" | 2025 | 2024 | 2023;

export default function KangoHistoryPage() {
  const router = useRouter();

  const [items, setItems] = useState<KangoHistoryItem[]>([]);
  const [isLoaded, setIsLoaded] = useState(false);
  const [yearFilter, setYearFilter] = useState<YearFilter>("all");

  useEffect(() => {
    const timer = window.setTimeout(() => {
      setItems(getKangoHistory());
      setIsLoaded(true);
    }, 0);
  ...
  const handleDeleteAll = () => {
    if (!window.confirm("看護師国家試験の採点履歴をすべて削除しますか？")) {
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 p-4">
      <div className="flex items-center justify-between gap-2">
        <h1 className="text-xl font-bold">看護師国家試験 採点履歴</h1>
      ...
      {!isLoaded ? null : filteredItems.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredItems.map((item) => (
            <div
              ...
              onClick={() => router.push(`/dashboard/kango/history/${item.id}`)}
            >
              ...
              <div className="mt-3 text-lg font-bold">
                {item.score} / {item.total} 点（{item.percent}%）
              </div>
